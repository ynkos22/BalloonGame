...
df = pd.read_csv("game_logs/game_23_log")


df_constant_pump = df[df["strategy_name"] == "Constant_pump_4"].groupby("balloon_number", as_index=False)["player_PnL"].first()
df_explore_exploit = df[df["strategy_name"] == "explore_exploit_2"].groupby("balloon_number", as_index=False)["player_PnL"].first()
df_thompson = df[df["strategy_name"] == "thompson_sampling"].groupby("balloon_number", as_index=False)["player_PnL"].first()
df_benchmark = df[df["strategy_name"] == "benchmark"].groupby("balloon_number", as_index=False)["player_PnL"].first()

x = df_constant_pump["balloon_number"]
y1 = df_benchmark["player_PnL"] - df_constant_pump["player_PnL"]
y2 =  df_benchmark["player_PnL"] - df_explore_exploit["player_PnL"]
y3 = df_benchmark["player_PnL"] - df_thompson["player_PnL"]
